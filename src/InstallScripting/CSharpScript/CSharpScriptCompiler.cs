...
namespace FomodInstaller.Scripting.CSharpScript
{
    ...
    public class CSharpScriptCompiler
    {
        ...
        public byte[] Compile(string p_strCode, Type p_tpeBaseScriptType, out IEnumerable<Diagnostic> p_cecErrors)
        {
            ...
            CSharpCompilation compilation = CSharpCompilation.Create("TestProgram", new[] {CSharpSyntaxTree.ParseText(p_strCode)}, referencedAssemblies
            ...
            {
                p_cecErrors = crsResults.Errors.Cast<CompilerError>()
                    .Select(e => Diagnostic.Create(
                        new DiagnosticDescriptor(
                            ...
                            e.ErrorText,
                            "Compiler",
                            e.IsWarning ? DiagnosticSeverity.Warning : DiagnosticSeverity.Error,
                            true),
                        Location.None))
                    .ToList();
                return null;
