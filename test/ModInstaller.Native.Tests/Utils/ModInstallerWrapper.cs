...
public sealed class ModInstallerWrapper
{
    ...
    public static unsafe return_value_json* ReadDirectoryFileList(param_ptr* handler, param_string* pDirectoryPath, param_string* pPattern, param_int searchOption)
    {
        try
        {
            ...
            var filesInFolder = modInstallerWrapper._data.ModArchive.Entries
                .Where(x => !x.IsDirectory && x.GetNormalizedName()?.StartsWith(directoryPath, StringComparison.OrdinalIgnoreCase) == true)
                .Select(x => x.GetNormalizedName())
                // TODO: Do something with pattern and searchOption
                .ToArray();

            return return_value_json.AsValue(filesInFolder, Utils2.CustomSourceGenerationContext.StringArray, false);
        }
        catch (Exception e)
        {
    ...
    public static unsafe return_value_json* ReadDirectoryList(param_ptr* handler, param_string* pDirectoryPath)
    {
        try
        {
            ...
            var foldersInFolder = modInstallerWrapper._data.ModArchive.Entries
                .Where(x => x.IsDirectory && x.GetNormalizedName()?.StartsWith(directoryPath, StringComparison.OrdinalIgnoreCase) == true)
                .Select(x => x.GetNormalizedName())
                // TODO: Do something with pattern and searchOption
                .ToArray();

            return return_value_json.AsValue(foldersInFolder, Utils2.CustomSourceGenerationContext.StringArray, false);
        }
        catch (Exception e)
        {
            return return_value_json.AsException(e, false);
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    public static unsafe return_value_string* ContextGetAppVersion(param_ptr* handler)
    {
        try
        {
            var modInstallerWrapper = (ModInstallerWrapper) GCHandle.FromIntPtr((IntPtr) handler).Target!;
            return return_value_string.AsValue(BUTR.NativeAOT.Shared.Utils.Copy(modInstallerWrapper._data.AppVersion, false), false);
        }
        catch (Exception e)
        {
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    public static unsafe return_value_string* ContextGetCurrentGameVersion(param_ptr* handler)
    {
        try
        {
            var modInstallerWrapper = (ModInstallerWrapper) GCHandle.FromIntPtr((IntPtr) handler).Target!;
            return return_value_string.AsValue(BUTR.NativeAOT.Shared.Utils.Copy(modInstallerWrapper._data.GameVersion, false), false);
        }
        catch (Exception e)
        {
            return return_value_string.AsException(e, false);
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    public static unsafe return_value_string* ContextGetExtenderVersion(param_ptr* handler, param_string* p_extender_name)
    {
        try
        {
            var modInstallerWrapper = (ModInstallerWrapper) GCHandle.FromIntPtr((IntPtr) handler).Target!;
            return return_value_string.AsValue(BUTR.NativeAOT.Shared.Utils.Copy(modInstallerWrapper._data.ExtenderVersion, false), false);
        }
        catch (Exception e)
        {
            return return_value_string.AsException(e, false);
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    public static unsafe return_value_void* UiEndDialog(
        param_ptr* handler)
    {
        try
        {
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    public static unsafe return_value_void* UiUpdateState(
        param_ptr* handler,
        ...
        param_int current_step)
    {
        try
        {
            ...
            if (modInstallerWrapper._unattended)
            {
                ...
                modInstallerWrapper._cont(modInstallerWrapper._callback_handler, true, currentStepValue, return_value_void.AsValue(false));
            }
            else if (modInstallerWrapper._dialogChoices is { Count: > 0 })
            {
                ...
                modInstallerWrapper._select(modInstallerWrapper._callback_handler, option.StepId, option.GroupId, pluginIds, return_value_void.AsValue(false));
